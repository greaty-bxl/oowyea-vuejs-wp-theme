...
final class OCEANWP_Theme_Class {
	...
	public static function pingback_header() {
		...
		if ( is_singular() && pings_open() ) {
			printf( '<link rel="pingback" href_2="%s">' . "\n", esc_url( get_bloginfo( 'pingback_url' ) ) );
		}
...
{		
		...
		?>
          <style type="text/css">
          	...
          	.elementor-6507 .elementor-element.elementor-element-1884e51 .htmega-tab-nav{

          		padding: 50px 0px 0px 0px !important;
          	}
          	.elementor-6493 .elementor-element.elementor-element-4c1561a .htmega-tab-nav{

          		padding: 50px 0px 0px 0px !important;
          	}
          	.elementor-6475 .elementor-element.elementor-element-eb19b50 .htmega-tab-nav{
...
add_action('wp_footer', 'grt_load_posts_slide');


function grt_load_posts_()
{	
		?>
			
		<!-- src="http://localhost/twaska/main.js" -->
			...
			<script >
						...
						jQuery(document).ready(function($) {
								...
								var hash_first_conditon = true
								var hash_first_conditon_2 = true

								var adresseActuelle = window.location;

								var passser = false;

								var first_tag = adresseActuelle.hash.split('/')[0]

								var second_tag = adresseActuelle.hash.split('/')[1]
								...
								jQuery(document).ready(function($) {
								 ...
								 if ( adresseActuelle.hash.split('/')[1] == '') {
							...
							$('.eael-tab-inline-icon').find('li').each(function(index, el) {


								href = $(this).find('span').text()
								...
								var index_interieur = $(this).attr('nbr');

								$('.clearfix:eq( ' + index_interieur +' )').addClass(href)
								...
								if ( first_tag == ''  ) {
									...
									if ( index == '0'  ) {
										...
										if (hash_first_conditon == true) {

										var url = document.location.href +achtag;
								...
								if ( achtag == first_tag) {
								    ...
								    setTimeout(function() {
								    	...
								    	var clicked_simulation = $('.eael-tab-inline-icon').find('li:eq( ' + index_interieur +' )').click();

								    	tag_init = clicked_simulation.attr('href');

								    }, 10);

								    $('.eael-tab-inline-icon').find('li').click(function(event) {
								    	  ...
								    	  setTimeout(function() {
								    	    ...
								    	    if ( $(this).hasClass('active')) {
								    	      	...
								    	      	if ( reload === true ) {
								    	      		...
								    	      		$('.eael-advance-tabs .eael-tabs-content>div.active, .eael-advance-tabs .eael-tabs-content>div.active-default').find("a:first").click()
											...
											var tag_init_2 = $(this).attr('href');

											var final_url="";

											final_url=oldURL.replace(tag_init, tag_init_2);
				                            ...
				                            var current_second_tag =  $('.htmega-tab-nav').find('a:eq( ' + index_interieur +' )')


				                            $('.htmega-tab-nav').find("[href_2= '" +second_tag+  "']").click()    
									  ...
									  current_second_tag =  $('.htmega-tab-nav').find('a:eq( ' + index_interieur +' )')
				...
				///// second tags /////
							...
							$('.htmega-tab-nav').find('a').each(function(index, el) {

								href_2 = $(this).text()

								href_2 = href_2.replace(/ /g, "-");
								href_2 = href_2.replace('é', "e");


								var achtag_2 =  '#' + href_2

								$(el).attr('href_2', achtag_2);

								$(this).attr('nbr', index);

								var index_interieur = $(this).attr('nbr');

								$('.clearfix:eq( ' + index_interieur +' )').addClass(href_2)


								if ( second_tag === undefined  ) {
									...
									if ( index == '0'  ) {
										...
										if (hash_first_conditon_2 == true) {


											if ( index == '0'  ) {


												second_tag_url    =  '/' + second_tag

												var url = document.location.href +second_tag_url;
									...
									second_tag = adresseActuelle.hash.split('/')[1]

								}


								if ( achtag_2 == second_tag) {

								    	var clicked_simulation_2 = $('.htmega-tab-nav').find('a:eq( ' + index_interieur +' )').click();


								    	tag_init_4  = clicked_simulation_2.attr('href_2');



								    $('.htmega-tab-nav').find('a').click(function(event) {

								    		tag_init_4 = adresseActuelle.hash.split('/')[1]

								    		href_2 = $(this).find('a').text()

											var oldURL = window.location.hash;

											var tag_init_3 = $(this).attr('href_2');


											second_tag = tag_init_3
											...
											var final_url="";
												...
												if (entree === true) {
													...
													final_url=oldURL.replace(  tag_init_4 ,  tag_init_5 );
													...
													// console.log(tag_init_3, 'tag_init_3');
													final_url=oldURL.replace(tag_init_4, tag_init_3);
													 history.pushState('data', '', final_url)
											...
											tag_init_4 = tag_init_4

											var href_2_tag_2 = '.' + href_2
...
add_action('wp_footer', 'grt_load_posts_');
